...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification
{
    ...
    if ((cps == nil) || (cps.count == 0)) {
        ...
        [selectionAlert beginSheetModalForWindow:_window
...
- (void)continueWithCameraSignature:(CoADeviceSignature *)deviceSignature
{
    ...
    if (_camera == nil) {
        [self awareToQuitByAlert:[NSString stringWithFormat:@"Can not create camera object for %@.",
    ...
    */
    ...
    //  it should be checked if image size is same as camera imager size.
    ...
    if (exprop != nil)
        [self.exposureTimeTextField
            setDoubleValue:((CoAExposureTimeAcquisitionProperty *)exprop).currentValue * 1000.0];
    ...
    if (_stream != nil) {
        ...
        [self awareToQuitByAlert:[NSString stringWithFormat:@"Can not create stream object for %@.",
                                                            deviceSignature.deviceId]];
        return;
    }
...
- (void)awareToQuitByAlert:(NSString *)messageText
{
    ...
    //  waiting user's awareness and giving time to put the alert sheet back.
...
- (void)stream:(CoAStream *)stream receiveBuffer:(CoABuffer *)buffer
{
    if ((buffer.payloadType == CoABufferPayloadTypeImage) ||
        ...
        [self performSelectorOnMainThread:@selector(receiveBufferFromFirstCamera:)
                               withObject:(CoAImageBuffer *)buffer
                            waitUntilDone:NO];
