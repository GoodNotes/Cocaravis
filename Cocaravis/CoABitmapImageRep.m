...
static UInt8 gamma2Table[] = {
    0,   1,   3,   5,   7,   9,   11,  13,  14,  16,  18,  20,  22,  24,  25,  27,  29,  31,  32,  34,  36,  38,
    39,  41,  43,  45,  46,  48,  50,  51,  53,  55,  56,  58,  60,  61,  63,  65,  66,  68,  70,  71,  73,  74,
    76,  78,  79,  81,  82,  84,  85,  87,  88,  90,  91,  93,  95,  96,  97,  99,  100, 102, 103, 105, 106, 108,
    109, 111, 112, 113, 115, 116, 118, 119, 120, 122, 123, 124, 126, 127, 128, 130, 131, 132, 134, 135, 136, 138,
    139, 140, 141, 143, 144, 145, 146, 148, 149, 150, 151, 153, 154, 155, 156, 157, 158, 160, 161, 162, 163, 164,
    165, 166, 168, 169, 170, 171, 172, 173, 174, 175, 176, 177, 178, 179, 180, 181, 182, 183, 184, 185, 186, 187,
    188, 189, 190, 191, 192, 193, 194, 195, 196, 197, 198, 199, 199, 200, 201, 202, 203, 204, 205, 205, 206, 207,
    208, 209, 210, 210, 211, 212, 213, 213, 214, 215, 216, 216, 217, 218, 219, 219, 220, 221, 221, 222, 223, 223,
    224, 225, 225, 226, 227, 227, 228, 229, 229, 230, 230, 231, 232, 232, 233, 233, 234, 234, 235, 235, 236, 236,
    237, 237, 238, 238, 239, 239, 240, 240, 241, 241, 242, 242, 243, 243, 243, 244, 244, 245, 245, 245, 246, 246,
    246, 247, 247, 247, 248, 248, 248, 249, 249, 249, 250, 250, 250, 250, 251, 251, 251, 251, 252, 252, 252, 252,
    252, 253, 253, 253, 253, 253, 253, 254, 254, 254, 254, 254, 254, 254, 255};
...
- (void)convertBayerImageBuffer:(CoAImageBuffer *)imageBuffer
    toShrinkedRGBAWithWhiteBalanceR:(float)wr
                                  G:(float)wg
                                  B:(float)wb;
                           ...
                           hasAlpha:(BOOL)alpha
{
    ...
    return [super initWithBitmapDataPlanes:NULL
                                pixelsWide:width
                                pixelsHigh:(NSInteger)(imageBuffer.imageSize.height)bitsPerSample:8
                           samplesPerPixel:spp
                                  hasAlpha:alpha
                                  isPlanar:NO
...
- (instancetype)initWithMonochromeImageBuffer:(CoAImageBuffer *)imageBuffer
{
    self = [self initWithImageBuffer:imageBuffer colorSpace:NSCalibratedWhiteColorSpace samplesPerPixel:1 hasAlpha:NO];
                                                    ...
                                                    withScaling:(double)scaling
{
    if (colorFormatTypeFromPixelFormat(imageBuffer.pixelFormat) != dgvColorFormatMonochrome)
        return nil;
    self = [self initWithImageBuffer:imageBuffer colorSpace:NSCalibratedRGBColorSpace samplesPerPixel:4 hasAlpha:YES];
...
- (instancetype)initWithBayerImageBuffer:(CoAImageBuffer *)imageBuffer
{
    //  TODO: Full-resolution Bayer not yet implemented
...
{
    return [self initWithShrinkedRGBAImageWithBayerImageBuffer:imageBuffer whiteBalanceR:1.0f G:1.0f B:1.0f];
}
- (instancetype)initWithShrinkedRGBAImageWithBayerImageBuffer:(CoAImageBuffer *)imageBuffer
                                                whiteBalanceR:(float)r
                                                            G:(float)g
                                                            B:(float)b
{
    ...
    self = [super initWithBitmapDataPlanes:NULL
                           ...
                           samplesPerPixel:4
                            ...
                            colorSpaceName:NSDeviceRGBColorSpace
                               bytesPerRow:width * 4
                              bitsPerPixel:0];
    [self convertBayerImageBuffer:imageBuffer toShrinkedRGBAWithWhiteBalanceR:r G:g B:b];
    return self;
}
